...
class Parser {
	...
	public function parseTuple(items:Array<Token>):Array<SelOp.SOValue> {
		var values:Array<SelOp.SOValue> = new Array();
		for (tk in items) {
			switch (tk) {
				case Token.TIdent(id):
					values.push(SelOp.SOValue.VReference(id));

				case Token.TString(str):
					values.push(SelOp.SOValue.VString(str));

				case Token.TNumber(num):
					values.push(SelOp.SOValue.VNumber(num));

				case Token.TTuple(itms):
					values.push(SelOp.SOValue.VArray(parseTuple(itms)));
					...
					case TIdent( id ):
						...
						switch (nxt) {
							case Token.TTuple(items):
								var args:Array<SelOp.SOValue> = parseTuple(items);
