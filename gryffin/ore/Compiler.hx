...
class Compiler {
	...
	public function resolveTuple(items:Array<SelOp.SOValue>):Array<Dynamic->Dynamic> {
		var results:Array<Dynamic->Dynamic> = new Array();
		for (item in items) {
			switch (item) {
				case SelOp.SOValue.VReference( id ):
					results.push(function(ent:Dynamic):Dynamic {
						return Reflect.getProperty(ent, id);
					});

				case SelOp.SOValue.VString(str):
					results.push(function(ent:Dynamic):Dynamic {
						return str;
					});

				case SelOp.SOValue.VNumber( num ):
					results.push(function(ent:Dynamic):Dynamic {
						return num;
					});

				case SelOp.SOValue.VArray( vals ):
					var getters:Array<Dynamic->Dynamic> = resolveTuple(vals);
					results.push(function(ent:Dynamic):Dynamic {
						return [for (getter in getters) getter(ent)];
					});
			}
		}
		return results;
	...
	public function compileOp( op:SelOp ):Dynamic -> Bool {
		switch ( op ) {
			...
			case SelOp.HelperFunctionCall(id, args):
				...
				var argGetters:Array<Dynamic->Dynamic> = resolveTuple(args);
				var rargs:Array<Dynamic->Dynamic> = [function(ent) return ent].concat(argGetters);

				return function( ent ) {
					return Reflect.callMethod(null, helper, [for (getter in rargs) getter(ent)]);
				};
