...
class Random {
	...
	public function new(?seed:Int):Void {
		this.state = (seed != null ? seed : Math.floor(Math.random() * GMath.INT_MAX));
	}
	public function nextInt():Int {
		this.state = cast ((1103515245.0 * this.state + 12345) % GMath.INT_MAX);
		return this.state;
	}
	public function nextFloat():Float {
		return (nextInt() / GMath.INT_MAX);
	}
	...
	public function choice <T> (set:Array<T>):T {
		return set[(randint(0, set.length))];
	}
	...
	public static function stringSeed(seed : String):Random {
		var state:Int = 0;
		var ba = gryffin.io.ByteArray.fromString(seed);
		for (bit in ba) {
			seed += bit.toInt();
		}
